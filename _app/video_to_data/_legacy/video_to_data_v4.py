...
def get_dominant_colors(image, num_colors=4):
    # Convert BGR to RGB for correct color handling
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

    # Apply Gaussian Blur to smooth the image
    image_rgb = cv2.GaussianBlur(image_rgb, (5, 5), 0)
    
    # Convert RGB to Lab color space for clustering
...
def crop_to_center(frame, crop_fraction=0.5):
    #this was 0.4, but that was too small, and 0.6 allows for black borders to exist

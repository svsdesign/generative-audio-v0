...
def create_midi_from_frames(json_data, midi_file_path, relevant_instruments, drum_tempo=None):
    print("create_midi_from_frames")  # Debugging line
    midi = MIDIFile(1)
    ...
    print("Instruments Used array", instruments_used)  # Debugging line
    ...
    for i, frame in enumerate(frames):
        ...
        print("Instruments Used:", instruments_used)  # Debugging line
    ...
    if request.method == 'POST':
        ...
        if json_file:
            ...
            if include_midi and midi_file:
                print("if include_midi and midi_file:")  # Debugging line

                midi_file_path = os.path.join(app.config['UPLOAD_FOLDER'], midi_file.filename)
                midi_file.save(midi_file_path)
                instruments_used = create_midi_from_frames(json_data, midi_file_path, relevant_instruments)
                print("Instruments Used:", instruments_used)  # Debugging line
            ...
            return render_template('result.html', file_name=file_name, 
                                ...
                                instruments_used=instruments_used)  # Pass instruments_used to the template
